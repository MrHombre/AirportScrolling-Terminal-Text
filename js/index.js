//Annotated code
var text = $(".switcher .shift").text().replace(/\ /g, '•').toUpperCase().split(','); //Get words, sort out spaces, put to uppercase and make an array.
var chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789•"; //allowed characters, case unspecific as it converts everything into uppercase.
...
function flicker() { //Matching characters
  ...
  if (latch) {
    $(".switcher .display").text(actualtext); //Display the (bird is the) word
  }
...
//log("Some text");
function log(i) { //used for displaying text in the top right
  $(".log").text(i);
}
